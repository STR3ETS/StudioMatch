<x-auth-layout :title="__('auth.register.title')">
    @php
        ...
        $role = request('rol') === 'verhuurder' ? 'verhuurder' : 'artiest';
    @endphp
    ...
    <form action="#" class="mt-8 space-y-5">
        ...
        <div>
            ...
            <input id="name" type="text" name="name" placeholder="{{ __('auth.fields.name_placeholder') }}" class="{{ $field }}" required>
        </div>

        <div>
            <label for="email" class="{{ $label }}">{{ __('auth.fields.email') }}</label>
            <input id="email" type="email" name="email" placeholder="{{ __('auth.fields.email_placeholder') }}" class="{{ $field }}" required>
        </div>
        ...
        <label class="flex cursor-pointer items-start gap-2.5 text-sm text-prussian-blue/80">
            <input type="checkbox" name="terms" class="mt-0.5 h-4 w-4 shrink-0 rounded border-prussian-blue/30 accent-ruby-red" required>
