...
<x-host-layout :title="__('invoice.host.title')" active="invoices">
    <h1 class="text-2xl font-bold text-prussian-blue">{{ __('invoice.host.title') }}</h1>
    <p class="mt-2 text-prussian-blue/60">{{ __('invoice.host.subtitle') }}</p>

    <x-info-note class="mt-6">{{ __('invoice.host.note') }}</x-info-note>

    @if ($bookings->isEmpty())
        <p class="mt-8 rounded-xl border border-dashed border-prussian-blue/20 bg-white px-4 py-6 text-center text-sm text-prussian-blue/50">{{ __('invoice.host.empty') }}</p>
    @else
            ...
            @foreach ($bookings as $booking)
                <div class="rounded-2xl border border-prussian-blue/10 bg-white p-5">
                    ...
                    <div class="mt-4 flex flex-wrap gap-2">
                        @foreach (\App\Support\Invoices::hostDocumentsFor($booking) as $type)
